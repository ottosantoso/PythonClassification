<!-- Main content -->
    <section class="content">
      ...
      <div class="col-xs-12">
       <div class="box-body">
        <?php $stt=0; if(!empty($list_data)){;foreach($list_data as $row2) {
          if($row2->status=="Disetujui"){
            $stt++;
          }
          }
        }
         ...
         <?= $this->session->flashdata('notif'); ?>
          ...
          <?php $i=0; if(!empty($list_data) and $stt==0){;foreach($list_data as $row) { ?>
            <tr>
            ...
            <th><input type="text" class="form-control" name="sikap<?php echo $i;?>" placeholder="sikap" value="<?php echo $row->sikap;?>"></th>
             <th>
             <?php if($row->tugas!=0&&$row->harian!=0&&$row->uts!=0&&$row->uas!=0&&$row->keterampilan!=0&&$row->sikap!=0){?>
                 <button class="btn btn-icon waves-effect waves-light btn-info m-b-5"> <i class="fa fa-fw fa-pencil"></i> 
                <a href="<?php echo base_url();?>index.php/c_deskripsi/form_deskripsi?idmapel=<?php echo $mapel; ?>&nis=<?php echo $row->nis; ?>">add</a></button>
